[TOC]
为了更好地满足各种不同的业务场景，DorisDB支持多种数据模型，DorisDB中存储的数据需要按照特定的模型进行组织（参考[表设计](3.表设计.md)章节）。数据导入功能是将原始数据按照相应的模型进行清洗转换并加载到DorisDB中，方便查询使用。
...
2.  **数据质量问题报错**：ETL\_QUALITY\_UNSATISFIED; msg:quality not good enough to cancel
