...
DorisDB中Range分布，被称之为分区，用于分布的列也被称之为分区列，图3-3中的event\_day就是分区列。Hash分布，则被称之为分桶，用于分布的列也被称之为分桶列，图3-2、3-3中分桶列都是site\_id.
...
:-: 图3.4：在site\_access表中添加新的分区，并使用新的分桶数   
