...
\[TOC\] 为了更好地满足各种不同的业务场景，DorisDB支持多种数据模型，DorisDB中存储的数据需要按照特定的模型进行组织（参考[表设计](../3.-biao-she-ji/)章节）。数据导入功能是将原始数据按照相应的模型进行清洗转换并加载到DorisDB中，方便查询使用。
...
* **Tablet**：DorisDB表的逻辑分片，一个表按照分区、分桶规则可以划分为多个分片（参考[数据分布](../3.-biao-she-ji/3.3-shu-ju-fen-bu.md)章节）。
...
1. 用户（外部系统）创建导入任务。
2. DorisDB返回导入结果。
3. 用户（外部系统）判断导入结果。如果导入结果为失败，可以再次创建导入任务。
...
4. 用户（外部系统）轮询查看任务状态，直到状态变为FINISHED或CANCELLED。
...
1. **数据质量问题报错**：ETL\_QUALITY\_UNSATISFIED; msg:quality not good enough to cancel
